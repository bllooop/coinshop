...
func NewShopPostgres(pg *pgxpool.Pool) *ShopPostgres {
	return &ShopPostgres{
		pg: pg,
	}
}
...
func (r *ShopPostgres) BuyItem(userid int, name string) (int, error) {
	...
	row = tr.QueryRow(context.Background(), createListQuery, userid, itemID, price, time.Now())
	if err := row.Scan(&id); err != nil {
		tr.Rollback(context.Background())
		return id, err
	}
	changeAmountQuery := fmt.Sprintf("UPDATE %s SET coins = $1 WHERE id = $2", userListTable)
	_, err = tr.Exec(context.Background(), changeAmountQuery, amount-price, userid)
	if err != nil {
		tr.Rollback(context.Background())
		return 00, err
	}
...
func (r *ShopPostgres) SendCoin(input domain.Transactions) (int, error) {
	...
	var id, amount int
	getCoinLeft := fmt.Sprintf("SELECT coins FROM %s WHERE id = $1", userListTable)
	row := tr.QueryRow(context.Background(), getCoinLeft, input.Source)
	if err := row.Scan(&amount); err != nil {
		return 0, err
	}
	...
	row = tr.QueryRow(context.Background(), sendMoneyQuery, input.Amount, input.Destination)
	if err := row.Scan(&id); err != nil {
		tr.Rollback(context.Background())
		return id, err
	}
	changeAmountQuery := fmt.Sprintf("UPDATE %s SET coins = coins - $1 WHERE id = $2", userListTable)
	_, err = tr.Exec(context.Background(), changeAmountQuery, input.Amount, input.Source)
	if err != nil {
		tr.Rollback(context.Background())
		return 00, err
	}
	...
	row = tr.QueryRow(context.Background(), createListQuery, input.Source, input.Destination, input.Amount, time.Now())
	if err := row.Scan(&id); err != nil {
		tr.Rollback(context.Background())
		return id, err
	}
